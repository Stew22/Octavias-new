...
begin
  ...
  begin
    ...
    if ExtractFileExt(dlgOpen1.FileName) <> '.csv' then
    begin
      ShowMessage('Please Select A .csv File For Import.');
      Exit; // Exit the procedure if the file format is not correct
    end;
    // Proceed with import if the file format is correct
    // Enable the button for updating pricing
    //btnupdatepricing.Enabled := True;
  end
  else
  begin
